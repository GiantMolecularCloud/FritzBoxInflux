...
class FritzBoxData:
    ...
    def __init__(self) -> None:
        ...
        """
        self.ip = os.getenv("FB_IP", "192.168.178.1")
        self.port = int(os.getenv("FB_PORT", "49000"))
        self.user = os.getenv("FB_USER")
        self.passwd = os.getenv("FB_PASSWD")
        self.id = os.getenv("FB_ID", "FritzBox")
        self.is_dsl = os.getenv("FB_DSL", True)
        self.timeout = 10.0
        self.data: dict = {}
        self.selected_data: dict = {}
        self.influx_data: list[dict] | None = None
        self.fritzbox: FritzConnection | None = None

    def connect(self) -> None:
        """
        Connect to the FritzBox using FritzConnection.
        """
        self.fritzbox = FritzConnection(
            address=self.ip,
            port=self.port,
            user=self.user,
    ...
    def _count_hosts(self) -> dict[str, int]:
        """
        ...
        for n in itertools.count():
            try:
                host = self.fritzbox.call_action("Hosts1", "GetGenericHostEntry", NewIndex=n)  # type:ignore[union-attr]
            except IndexError:
                break
            hosts_known = hosts_known + 1
            if host["NewActive"]:
                hosts_active = hosts_active + 1
                if host["NewInterfaceType"] == "Ethernet":
                    lan_hosts_active = lan_hosts_active + 1
                if host["NewInterfaceType"] == "802.11":
                    wlan_hosts_active = wlan_hosts_active + 1
            if host["NewInterfaceType"] == "Ethernet":
                lan_hosts = lan_hosts + 1
            if host["NewInterfaceType"] == "802.11":
                wlan_hosts = wlan_hosts + 1
        hosts = {
            "HostsKnown": hosts_known,
            "HostsActive": hosts_active,
            "HostsKnownLAN": lan_hosts,
            "HostsActiveLAN": lan_hosts_active,
            "HostsKnownWLAN": wlan_hosts,
            "HostsActiveWLAN": wlan_hosts_active,
        }
        return hosts
    ...
    def read_data(self) -> None:
        ...
        for [name, module, action] in [
            ["deviceInfo", "DeviceInfo1", "GetInfo"],
    ...
    def select_data(self) -> None:
        ...
        try:
            self.selected_data["device"] = {
                "model": self.data["deviceInfo"]["NewModelName"],
                "firmware": self.fritzbox.device_manager.system_version,  # type:ignore[union-attr]
        ...
        try:
            self.selected_data["connection"] = {
                "connection_time": self.data["connectionInfo"]["NewUptime"],
        ...
        try:
            self.selected_data["traffic"] = {
                "rate_byte_down": self.data["trafficInfo"]["NewByteReceiveRate"],
        ...
        try:
            self.selected_data["network"] = {
                "hosts_known": self.data["dhcpInfo"]["NewHostNumberOfEntries"],
    ...
    def format_data(self) -> None:
        ...
        self.influx_data = [
            {"measurement": groupname, "time": self.time, "fields": group}
    ...
    def get_influx_data(self) -> list[dict]:
        ...
        self.select_data()
        self.format_data()
        return self.influx_data  # type:ignore[return-value]
